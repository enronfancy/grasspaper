...
In order to measure the effectiveness of our skeleton refining algorithm, we use a Hausdorff-Fit method to evaluate the refining result. We intend to measure the "distance" between refined skeleton and the original one before refining. Hausdorff distance is usually used to measure how far two subsets of a metric space are from each other\cite{wikihausdorff}. Hausdorff distance is athe maximum distance of a set to the nearest point in the other set.
...
where $sup$ represents the supremum of the set and $inf$ represents the infimum. In our method, Hausdorff distance is used to measure the similarity of refined skeleton and the original one before refining. However, since the skeleton after refining could be a sub set of the original one, then hausdorff distance could become meaningless in that case. Therefore we eliminate vertices from original skeleton vertex set $\alpha$ if it is also in refined skeleton vertex set $\beta$, then we calculate the hausdorff distance from $\alpha$ to $\beta$. In this way we actually measure the similarity between the culled vertex set and the refined result. Algorithm for our method is demonstrated in algorithm.\ref{tbl:skeletonrefiningeva} where $d_H$ is refined hausdorff distance used in our algorithm.\\
...
We extracted several frame data according to our simulation algorithm and for each frame, we are able to obtain original skeleton vertices information and refined skeleton vertices information. Then we could calculate $d_H$ as in algorithm.\ref{tbl:skeletonrefiningeva} in each frame for the results of different refining methods. From Figure.\ref{fig:evaresults} we could see that our skeleton refining algorithm always get the shortest distance between original skeleton and refined result.


\begin{figure}
    \centering
    \includegraphics[width=0.48\textwidth]{figs/test3.jpg}
    \captionof{figure}{Skeleton refining evaluation results: }
